...
export function HudReactor({ level = 0, state = "idle" as JefreyState, onClick }: { level?: number; state?: JefreyState; onClick?: () => void }) {
  ...
  const [appearance, setAppearance] = useState<"light" | "dark" | "stark">("stark")
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-strong border-indigo-500/20 rounded-xl p-3 mb-3">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-indigo-300">Aparência</span>
          <button
            onClick={() =>
              setAppearance(prev => {
                if (prev === "light") return "dark"
                if (prev === "dark") return "stark"
                return "light"
              })
            }
        ...
        <div className="space-y-2">
          <label className="text-[9px] text-cyan-400 uppercase tracking-wider">Hue</label>
          <div className="flex gap-1">
            <motion.button
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className="w-8 h-8 rounded-full border-2 border-indigo-500/30 bg-indigo-500/10 flex items-center justify-center text-indigo-400 text-xs font-mono"
              style={{ transition: "transform 80ms ease-out" }}
              onClick={() => setHue((prev: number) => (prev + 30) % 360)}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <circle cx="12" cy="12" r="10" />
                <path d="M8 12l4-4l8 8L8 12z" />
              </svg>
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className="w-8 h-8 rounded-full border-2 border-green-500/30 bg-green-500/10 flex items-center justify-center text-green-400 text-xs font-mono"
              style={{ transition: "transform 80ms ease-out" }}
              onClick={() => setHue((prev: number) => (prev + 150) % 360)}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <circle cx="12" cy="12" r="10" />
                <path d="M8 12l4-4l8 8L8 12z" />
            ...
            <motion.button
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className="w-8 h-8 rounded-full border-2 border-purple-500/30 bg-purple-500/10 flex items-center justify-center text-purple-400 text-xs font-mono"
              style={{ transition: "transform 80ms ease-out" }}
              onClick={() => setHue((prev: number) => (prev + 270) % 360)}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <circle cx="12" cy="12" r="10" />
                <path d="M8 12l4-4l8 8L8 12z" />
        ...
        {/* Animation toggle */}
        <div className="mt-2">
          <label className="text-[9px] text-cyan-400 uppercase tracking-wider">
            <input
              type="checkbox"
              checked={animEnabled}
              onChange={e => setAnimEnabled(e.target.checked)}
              className="w-4 h-4 rounded border-indigo-500 focus:ring-indigo-500"
            />
            Animações
          </label>
        </div>
      ...
      {/* HUD Reactor principal */}
      <div className="flex flex-col items-center gap-2">
        <button
          aria-label="Hud reactor — clique para falar com Jefrey"
          onClick={onClick}
          className={`relative flex items-center justify-center rounded-full hud-ring hud-pulse bg-gradient-to-br from-cyan-500/10 to-blue-500/10 glass overflow-hidden ${animEnabled ? "" : "animation-none"}`}
          style={{
            width: 180,
            height: 180,
        ...
        >
          ...
          {state === "thinking" && (
            <motion.svg
              whileHover={{ rotate: spin + 10 }}
              whileTap={{ rotate: spin - 10 }}
            ...
            >
              <g transform="translate(90, 90)">
                {[0, 120, 240].map(a => (
                  <path
                    key={a}
                    d="M 0 -62 A 62 62 0 0 1 54 -31"
                    fill="none"
                    stroke="rgba(34,211,238,0.9)"
                    strokeWidth="2.2"
                    strokeLinecap="round"
                    transform={`rotate(${spin + a})`}
                  />
                ))}
              </g>
            </motion.svg>
          ...
          {state === "speaking" && (
            <motion.div
              className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-end gap-[3px] h-6"
            >
              {bars.map((h, i) => (
                <span key={i} className="w-[3px] rounded-full bg-indigo-400/90" style={{ height: h, opacity: 0.9 - i * 0.04 }} />
              ))}
            </motion.div>
          )}
          ...
          {/* chime indicator */}
          {animEnabled && state !== "idle" && state !== "asleep" && (
            <span className="absolute top-2 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-lime-500/60 animate-bounce" />
          )}
        </button>
