    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            mr.onstop = async () => {
                setState("processing");
                try {
                    window.__setHudState?.("thinking");
                }
                catch { }
                try {
                    const blob = new Blob(chunksRef.current, { type: mime });
                    if (blob.size < 500)
                        throw new Error("audio muito curto");
                    // POST /stt â€” DRY via authHeaders (Axiom #2 X-User-Id obrigatorio)
                    const fd = new FormData();
                    fd.append("audio", blob, "audio.webm");
                    const headers = authHeaders();
                    const r = await fetch("/stt", { method: "POST", headers: headers, body: fd });
                    if (!r.ok) {
                        const j = await r.json().catch(() => ({ detail: r.statusText }));
                        throw new Error(j.detail || `STT ${r.status} ${r.statusText}`);
                    }
                    const j = await r.json();
                    const txt = j.transcript || j.text || "";
                    if (!txt)
                    ...
                    if (chatR.ok) {
                        ...
                        if (cj.status === "complete")
                            replyText = cj.response || cj.message || txt;
                        else if (cj.status === "running") {
                            for (let i = 0; i < 40; i++) {
                                await new Promise((r) => setTimeout(r, 1500));
                                const pr = await fetch(`/chat/status/${encodeURIComponent(threadId)}`, { headers: authHeaders() });
                                if (!pr.ok)
                                    continue;
                                const pj = await pr.json().catch(() => ({}));
                                if (pj.status === "complete") {
                                    replyText = pj.response || pj.message || txt;
                                    break;
                                }
                                if (pj.status === "error")
                                    break;
            ...
            setState("recording");
            try {
                window.__setHudState?.("listening");
            }
            catch { }
        }
        catch (e) {
