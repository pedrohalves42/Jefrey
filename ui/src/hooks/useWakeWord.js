...
function isWakeDetected(textLower, keyword, aliases, ratio = 0.78) {
    const all = new Set([keyword, ...aliases]);
    if (textLower.includes(keyword))
        return true;
    for (const al of aliases)
        if (textLower.includes(al))
            return true;
    const tokens = textLower.split(/\s+/).map(t => t.replace(/^[.,!?;:"'()\[\]{}]+|[.,!?;:"'()\[\]{}]+$/g, "")).filter(Boolean);
    for (const token of tokens) {
        for (const alias of all) {
            if (fuzzyRatio(alias, token) >= ratio)
                return true;
        }
    }
    return false;
...
export function useWakeWord(opts) {
    ...
    useEffect(() => { setSupported(!!(window.webkitSpeechRecognition || window.SpeechRecognition)); }, []);
            ...
            rec.onresult = (e) => {
                for (let i = e.resultIndex; i < e.results.length; i++) {
                    ...
                    if (isWakeDetected(txt, keyword, aliases)) {
                        ...
                        // chime feedback
                        try {
                            new Audio("/vite.svg").play().catch(() => { });
                        }
                        catch { }
                    }
                }
            };
            rec.onend = () => { if (enabled)
                try {
                    rec.start();
                }
                catch { } };
