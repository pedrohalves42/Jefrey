"""Validação final da Fase 1 - Componentes essenciais."""
import asyncio
import sys
import os
...
async def main():
    print("=" * 70)
    print("VALIDAÇÃO FINAL JEFREY - FASE 1")
    print("=" * 70)
    ...
    # Apenas ferramentas essenciais (sem calendar/email, que estão desabilitadas)
    ...
    missing = [t for t in essential_tools if not any(t in tool.name for tool in agent.tools)]
    print(f"   - Ferramentas essenciais presentes: {len(essential_tools) - len(missing)}/{len(essential_tools)}")
    ...
    # Teste via Ollama (não via LangGraph)
